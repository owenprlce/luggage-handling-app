...
function FlightPassengersTable({ flight, onBack }) {
    ...
    const { passengers, setPassengers, setMessages, bags, setBags, currentUser, alerted, setAlerted } = useData()
    ...
    const allPassengersBoarded = flightPassengers.every(p => p.status === "Boarded");
    const allBagsLoaded = bags
        .filter(b => flight.ticketNumbers.includes(b.ticketNumber))
        .every(b => b.location === `LOADED-${flight.flightId}`);
    ...
    const handleBoardPassenger = (passenger) => {
        const getPassengerBags = bags.filter(b => b.ticketNumber === passenger.ticketNumber);
        console.log(getPassengerBags);

        const gateLocation = `GATE-${flight.gateInformation.terminal}${flight.gateInformation.gateNumber}`

        console.log(gateLocation);

        const bagsAtGate = getPassengerBags.every(b => b.location === gateLocation);

        console.log(bagsAtGate);

        if (!bagsAtGate) {
            // Alert
            console.log("Not all here");
            return;
        }

        setPassengers(_passenger => _passenger.map(p => p.ticketNumber === passenger.ticketNumber ?
            { ...p, status: "Boarded" } : p
    ...
    const isPassengerReadyToBoard = (passenger) => {
        ...
        const bagsAtGate = passengerBags.every(b => b.location === gateLocation);

        return passenger.status === "Boarded" || bagsAtGate;
    };


    function alertAdmin(flightInformation) {



        if (!canAlertAdmin) {
            // Alert
            console.log("Not all bags loaded, flight cannot take off");
            return;
        } else {
            setMessages(messages => [
                ...messages,
                {
                    msg: `Flight ${flightInformation.flightId} Clear For Takeoff`,
                    from: `GATE`,
            ...
            setErrorMessage(`Flight (${flightInformation.flightId}): clear for takeoff`)
            setErrorMessageState(true)
        }
                            ...
                            {canAlertAdmin && (
                                <div className="flex flex-row">
                                    <button
                                        ...
                                        onClick={() => alertAdmin(flight)}>Alert Admin
                                    </button>
                                    <div>
                                        ...
                                        <td className="p-4">
                                            {
                                                passenger.status === "Not-checked-in" || !isPassengerReadyToBoard(passenger) ? (
                                                    <button
                                                        className="bg-red-400 animate-pulse text-white rounded-full"
                                                    >
